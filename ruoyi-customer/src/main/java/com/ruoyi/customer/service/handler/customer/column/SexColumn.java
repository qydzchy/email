...
@Slf4j
@Component
public class SexColumn extends ColumnAbstract implements IColumnService {

    @Override
    public CustomerColumnEnum getCustomerColumnEnum() {
        return CustomerColumnEnum.SEX;
    }
    ...
    public boolean handler(CustomerDetailVO customerDetail, SegmentConditionRuleBO segmentConditionRule) {
        ...
        if (contactList != null && !contactList.isEmpty()) {
            for (CustomerContactBO customerContactBO : contactList) {
                if (super.inNullIntegerHandler(customerContactBO.getSex(), segmentConditionRule)) {
                    return true;
                }
            }
        }
