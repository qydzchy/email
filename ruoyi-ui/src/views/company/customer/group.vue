<template>
	<div class="pool-group">
		<div class="info-tip flex-middle">
			<div>
				<i
					class="el-icon-warning mr-6"
					style="color: #1c76fd"></i>
			</div>
			<div
				class="fs-12 wrap"
				style="color: #666666">
				<p>
					对客户资源进行分层分级管理，可迅速区分客户类型，针对性地跟进维系。大部分企业采取以下方式分组，供您参考：
				</p>
				<p>
					1、按照客户重要程度：潜在客户\意向客户\初次成交\多次成交\VIP客户
				</p>
				<p>
					2、按照客户交易推进程度：潜在客户\营销中客户\意向客户\样品客户\合作客户\复购客户
				</p>
				<p>3、按照客户地理位置</p>
				<p>4、按照公司产品线</p>
			</div>
				...
				<template v-slot="scope">
					<el-row>
						...
						<el-button
							size="mini"
							type="text"
							@click="editGroupTable(scope.row)">
							编辑
						</el-button>
						<DelPopover
							:id="scope.row.id"
							width="200"
							:content="`确定要删除分组【${scope.row.name}】吗？`"
							@onDelete="onDelete" />
					</el-row>
				</template>
			</el-table-column>
					...
					<div class="flex-column">
						<div>
							...
							<el-tooltip placement="top">
								<div slot="content">
									<div
										class="pool-rule-tooltip fs-14 lineH-24"
										style="width: 300px">
		...
		methods: {
			...
			async getCommonTree() {
				try {
					...
					if (res.code === 200) {
						...
						this.initGenerateMemberData(res.data);
					}
				} catch {}
			},
			// 表格添加组
			addGroupTable(row) {
				const searchName = this.generateSearchParent(
					this.menuList,
					row?.parentId,
				);
				const parentName =
					row?.parentId !== -1 ? searchName : '客户分组';
			...
			editGroupTable(row) {
				...
				this.groupDialogForm = {
					...
					parentName: parentName,
					designatedMember:
						this.generateMemberChoose(designatedMember),
				};
				this.groupDialog = true;
			},
			async onDelete(id) {
			...
			async packetAddReq(row) {
				try {
					...
					const res = await packetAdd({
						...
						availableMember,
						designatedMember:
							availableMember === 2 ? designatedMember : '',
					});
					if (res.code === 200) {
						this.$message({
							type: 'success',
			...
			// 确认
			onConfirm() {
				const { name, designatedMember, availableMember } =
					this.groupDialogForm;
				if (!name) {
					this.$message({
						type: 'error',
					...
					return;
				} else if (availableMember === 2 && !designatedMember?.length) {
					this.$message({
						type: 'error',
						message: '请选择成员',
					});
				...
				const ids = this.formatDesignatedMember(designatedMember);
				const formData = {
					...this.groupDialogForm,
					designatedMember: this.generateMemberFormat(ids),
				};
				if (!formData.id) {
			...
			// 提交前处理designatedMember数据
			formatDesignatedMember(arr) {
				if (arr && !arr.length) {
					return [];
				}
				let chooseIds = [];
				arr.forEach((val) => {
					chooseIds = chooseIds.concat(val);
				});
				chooseIds = [...new Set(chooseIds)];
				return chooseIds;
			},
			...
			editSettings() {
				this.$store
					.dispatch('company/EditCompanyCustomerSettings', {
						...this.settings,
			...
			generateMemberChoose(scopeData) {
				...
				const searchParentId = (data, value) => {
					for (let i in data) {
						if (data[i].id === value) {
							return [data[i].id];
						}
						if (data[i].children) {
							let node = searchParentId(data[i].children, value);
							if (node) {
								node.unshift(data[i].id);
								return node;
							}
						}
					}
				...
				if (scope?.user?.allFlag) {
					this.tempAllUserIds.forEach((val) => {
						const searchResult = searchParentId(
							this.memberOption,
							val,
						);
					...
					scope?.user?.userIds.forEach((val) => {
						const searchResult = searchParentId(
							this.memberOption,
							val,
						);
			...
			generateMemberOption(list) {
				...
				const result = deepSearch(list);
				return result;
			},
			// 格式化成员结构
			initGenerateMemberData(list) {
				let ids = [];
				let allDept = {
					id: 'allDept',
					name: '全部部门',
			...
			generateMemberFormat(ids) {
				...
				// 校验勾选部门
				const validDept = this.tempAllDept.every((val) =>
					ids.includes(val.id),
				);
				if (validDept) {
					...
					let deptIds = this.tempAllDept.filter((val) =>
						ids.includes(val.id),
					);
					templateScope.dept.deptIds = deptIds.map((val) => val.id);
				}
				// 校验勾选用户
				const validUser = this.tempAllUser.every((val) =>
					ids.includes(val.id),
				);
				if (validUser) {
					...
					let userIds = this.tempAllUser.filter((val) =>
						ids.includes(val.id),
					);
